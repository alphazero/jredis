 ...
 * 
 */
public class DefaultCodec implements Serializable{
...
//	static final Codec<Serializable> java_lang_Object_codec = DefaultCodec.getDefaultObjectCodec(Serializable.class);
	
//	public final static <T extends Object> Codec<T> getDefaultObjectCodec (Class<T> clazz) {
//		return new Codec<T> () {
//			@Override
//			public T decode(byte[] bytes) { return DefaultCodec.decode(bytes); }
//			
//			@Override
//			public byte[] encode(T object) { return DefaultCodec.encode(object); }
//		};
//	}
	...
	public static final List<Integer> toInt (List<byte[]> bytearray) {
		List<Integer> list = new ArrayList<Integer>(bytearray.size());
		for(byte[] b : bytearray) list.add(toInt(b));
		return list;
	}
